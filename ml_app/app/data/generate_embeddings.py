...
class SaveEmbeddings(BaseEstimator, TransformerMixin):
    def __init__(self, n):
        self.path = os.path.join(PACKAGE_ROOT, f'embeddings_{n}d.faiss')
...
class CustomPipeline():
    def __init__(self, n):
        self.pipeline = Pipeline([
                    ...
                    ('DimensionalityReduction', DimensionalityReduction(n)),
                    ('StandarScaling_2', StandardScaling()),
                    ('SaveEmbeddings', SaveEmbeddings(n))
        ])

# Set up argument parser for the target parameter
parser = argparse.ArgumentParser(description="Generate embeddings for the ml app visualization.")
parser.add_argument('n_components', type=int, help="Specify the number of components to reduce the embeddings.")
...
custom_pipeline.pipeline.fit(words)
